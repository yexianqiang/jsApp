<template>
  <div class="audio-list-view">
    ...
    <transition name="slide">
      <div class="tags-panel-wrap" v-if="isShowPanel">
        ...
        <div class="tags-panel" v-if="showChildrenTags">
            ...
            <ul class="theme-tags">
              <template v-for="(item,index) in childTags">
                <li :class="{'active':item.active}" @click="selectTags(item,index)">{{item.name}}</li>
              </template>
            </ul>
        </div>
        ...
        <div class="tags-panel" v-if="showParentTags">
          ...
          <ul class="theme-tags">
            <template v-for="(item,index) in parentTags">
              <li :class="{'active':item.active}" @click="selectTags(item,index)">{{item.name}}</li>
            </template>
          </ul>
        </div>
    ...
    <scroller style="padding-top: 0.8rem;" :on-refresh="refresh" :on-infinite="infinite" :noDataText="noDataText" ref="my_scroller">
      <ul class="audio-list">
          ...
          <li @click="showDetail(item)">
            ...
            <div class="audio-intro">
              ...
              <div class="audio-intro-bottom">
                ...
                <div class="audio-price" v-if="item.price_curr == 0">
...
<style lang="scss">
  ...
  .audio-list-view{
    ...
    .audio-list-nav{
      ...
      &>li{
        font-size: 0.3rem;color:#222;margin: 0 0.39rem;line-height: 0.8rem;flex-shrink: 0;position: relative;
        &.active:after{
...
//      上拉加载，下拉刷新的配置
    ...
    mounted() {
      let vthis = this;
      this.getNavTags();
//  通过判断路由的名字来决定该页面展示的是视频列表（type为2）还是音频列表(type为1)
      this.type = this.$route.name == 'subject.audio.list' ? 1 : 2;

      if (this.type == 1) {
        this.nav = ['全部','免费','家长','孩子','职场自助']
        document.title = '音频课程';
      } else if (this.type == 2) {
        this.nav = ['全部','免费','家长']
        document.title = '视频课程';
    ...
    methods: {
      ...
      initSpecialPage() {
        ...
        if (this.$route.query.channel == 5) { // 家长课程
          this.criterion.push(65);
          this.getListFirst();
          this.nav_selected = 2;

        } else if(this.$route.query.channel == 6) { // 孩子课程
          this.criterion.push(66);
          this.getListFirst();
          this.nav_selected = 3;
      ...
      getNavTags() {
        ...
        if (tagsSession) {
          response = JSON.parse(tagsSession).data;
          response.forEach(function (item ,index) {
            if (item.id == 65) { // 表明为家长
              vthis.parentTags = item.attr_list;
            } else if (item.id == 66) { // 表明为孩子
              vthis.childTags = item.attr_list;
            } else if (item.id == 67) { // 表明为职场自助

            }
          })
        } else {
          this.$http.post(zzhJs.config.apiDomain + '/zzhedu/wx/product/product/attr.action').then(res => {
      ...
      selectNav(index,item) { // 选择导航栏中的标签来切换面板
        ...
        if (item == '家长') {
          ...
          if (this.nav_selected == index) {
            ...
            this.clearSelected(this.parentTags);
          }
          ...
          if (this.nav_selected == index){
            ...
            this.isShowPanel = true;
            this.clearSelected(this.childTags);
          }
          this.nav_selected = index;
          ...
          this.criterion = [];
          this.getListFirst({ free: 1});
          this.nav_selected = index;
        } else if (item == '免费') {
          this.isShowPanel =  false;
          this.criterion = [];
          this.getListFirst({ free: 0})
          this.nav_selected = index;
        } else if (item == '职场自助') {
          this.criterion = [];
          this.isShowPanel =  false;
...
//        this.nav_selected = index;
      ...
      selectTags(item,index) {
        ...
        if (this.nav_selected == 2) {
          this.$set(this.parentTags,index,item)
        } else if (this.nav_selected == 3) {
          this.$set(this.childTags,index,item)
        }
      ...
      confirmTags() {
        let vthis = this;
        this.isShowPanel = false;
        if (this.nav_selected == 2) { // 表明当前是请求的家长的
      ...
      getList(params,callback){
        ...
        this.weLoadStart();
        this.$http.post(zzhJs.config.apiDomain + '/zzhedu/wx/product/audioAndVideo.action', params).then(res => {
          let response = res.body.data;
          vthis.noInfo = response.total <= 0 ? true : false;
          vthis.noDataText = res.body.data.total <= 0 ? '':'暂无更多数据';

          vthis.counter = res.body.data.page_no;
          vthis.pageEnd = Math.ceil(res.body.data.total / vthis.num);
          response.list.forEach(function (e) {
            vthis.data_list.push(e);
          })
      ...
      showDetail(item) {
        ...
        if (zzhJs.env.isInApp()) {
          if (this.type == 1) { // 音频
            ...
            // 新的js与app交互接口
//            zzhJs.messageBridge.sendToApp({type:'showAudioDetail',data:{id:item.id}});
            ...
            // 新的js与app交互接口
//            zzhJs.messageBridge.sendToApp({type:'showVideoDetail',data:{id:item.id}});
          }
        } else {
          if (this.type == 1) {// 音频
